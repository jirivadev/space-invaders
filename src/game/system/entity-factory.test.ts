...
import { damageShieldRect, createExplosionParticles, createShield, createAliens } from './entity-factory';
...
describe('damageShieldRect', () => {
  ...
  it('handles negative coordinates without throwing', () => {
    ...
    expect(() => damageShieldRect(shield, shield.x - 10, shield.y - 10, 30, 30)).not.toThrow();
  ...
  it('handles zero width or height without throwing', () => {
    ...
    expect(() => damageShieldRect(shield, shield.x, shield.y, 0, 10)).not.toThrow();
    expect(() => damageShieldRect(shield, shield.x, shield.y, 10, 0)).not.toThrow();
  });
...
describe('createAliens', () => {
  const formations = ['grid', 'staggered', 'diamond', 'compact', 'wide'] as const;
