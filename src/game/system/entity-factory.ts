...
export function createAliens(
  ...
  const rowTypes: ("squid" | "crab" | "octopus")[] = [
    "squid",
    "crab",
    "crab",
    "octopus",
  ...
  switch (formation) {
    case "staggered": {
      const spacingX = 48;
      return createFormationGrid(
        5,
        11,
        spacingX,
        40,
        () => 80,
        (r) => (r % 2 === 0 ? 0 : spacingX / 2),
        rowTypes,
        startY,
        scale
      );
    ...
    case "diamond": {
      ...
      return createFormationGrid(
        ...
        48,
        40,
        (_r, count) => baseX + ((maxCols - count) * 48) / 2,
        noop,
        rowTypes,
        startY,
        scale
...
export function damageShieldRect(
  shield: Shield,
  x: number,
  y: number,
  w: number,
  ...
  const right = Math.min(
    shield.cols - 1,
    Math.floor((x + w - shield.x) / shield.pixelSize)
  );
  const top = Math.max(0, Math.floor((y - shield.y) / shield.pixelSize));
  const bottom = Math.min(
    shield.rows - 1,
    Math.floor((y + h - shield.y) / shield.pixelSize)
  );
