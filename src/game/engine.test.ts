...
describe("GameEngine smoke", () => {
  ...
  beforeEach(() => {
    ...
    const _cAF: typeof cancelAnimationFrame = () => {
    ...
    (globalThis as unknown as Record<string, unknown>).__rAF_callback =
      capturedCallback;

    // Stub window for InputHandler (which calls window.addEventListener / removeEventListener)
    vi.stubGlobal(
      "window",
      Object.assign(typeof window !== "undefined" ? window : {}, {
  ...
  it("submitLeaderboard writes through to localStorage", () => {
    ...
    expect(parsed).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "TestPlayer", score: 5000 }),
      ])
    );
  ...
  it("_handleStateTransitions is reachable via status changes", () => {
    ...
    const engineAny = engine as unknown as {
      _frame(): void;
      g: { status: string; keys: Record<string, boolean> };
    };
    engineAny._frame();
